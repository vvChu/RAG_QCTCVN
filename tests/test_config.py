...
class TestConfig:
    ...
    def mock_env(self):
        with patch.dict(
            os.environ,
            {"GEMINI_API_KEY": "test_key", "MILVUS_HOST": "localhost", "MILVUS_PORT": "19530"},
            clear=True,
        ):
            yield
    ...
    def test_settings_missing_required(self):
        if os.path.exists(".env"):
            pytest.skip(
                "Skipping test because .env file exists and interferes with Pydantic settings loading"
            )
    ...
    def test_settings_type_conversion(self):
        with patch.dict(
            os.environ,
            {"GEMINI_API_KEY": "test_key", "MILVUS_HNSW_M": "16", "BGE_USE_FP16": "False"},
        ):
            import importlib
